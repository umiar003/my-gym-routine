...
export default function SignupPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setFeedback(
      "We emailed you a confirmation link. Please verify your address before signing in."
    );
    setIsSubmitting(false);
  };
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-sky-50 via-white to-slate-100 px-4 py-12 dark:from-slate-950 dark:via-slate-900 dark:to-slate-900">
      <AuthCard
        ...
        description="Sign up to start building your repeatable cycles."
        footer={
      ...
      >
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          {feedback ? (
            <p className="rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-700 dark:border-sky-800 dark:bg-sky-950/40 dark:text-sky-300">
              {feedback}
            </p>
          ) : null}
